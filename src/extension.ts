...
function getConfig() {
  ...
  const apiUrl = config.get<string>('apiUrl') ?? 'http://localhost:11434/v1/chat/completions'
 ...
 */
async function requestCompletion(prefix: string, suffix: string): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[local-llm-code-completion] API error:', response.status, response.statusText)
      return null
    }
...
const inlineProvider: vscode.InlineCompletionItemProvider = {
  async provideInlineCompletionItems(
    ...
    const endPos = new vscode.Position(endLine, document.lineAt(endLine).range.end.character)
    ...
    // 生成中通知付き
    const result = await vscode.window.withProgress<vscode.InlineCompletionList | undefined>(
      {
      ...
      async (_progress, progressToken) => {
        ...
        console.log('[local-llm-code-completion] completion text:', JSON.stringify(completion))
...
export function activate(context: vscode.ExtensionContext) {
  vscode.window.showInformationMessage('local-llm-code-completion ACTIVATED!')

  // Hello World コマンド（動作確認用）
  const disposable = vscode.commands.registerCommand('local-llm-code-completion.helloWorld', () => {
    vscode.window.showInformationMessage('Hello from Local LLM Code Completion!')
  })
  context.subscriptions.push(disposable)

  // ★ 追加：Progress通知のテストコマンド
  const testProgress = vscode.commands.registerCommand('local-llm-code-completion.testProgress', async () => {
  ...
  const providerDisposable = vscode.languages.registerInlineCompletionItemProvider(selector, inlineProvider)
